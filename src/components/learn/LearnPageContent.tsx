...
import { stageConfig, moduleConfig, getModuleLessons, difficultyConfig } from '@/data/curriculum';
...
export default function LearnPage() {
  const [activeTab, setActiveTab] = useState<'stages' | 'modules' | 'progress'>('stages');

  return (
    <div className="p-8">
      {/* 页面标题 */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">📚 AI博士学院</h1>
                  ...
                  {stage.modules.map((moduleId) => {
                    ...
                    return (
                      <div key={moduleId} className="flex items-center gap-2 text-sm">
                        ...
                        <span className="text-gray-700">{module?.title ?? moduleId}</span>
                      </div>
                    );
                  })}
                </div>
          ...
          {stages.map((stage) => (
            <div key={stage.id}>
              <div className="flex items-center gap-2 mb-4">
                <div className={`w-8 h-8 rounded-lg bg-gradient-to-r ${stage.gradient} flex items-center justify-center text-white`}>
                  {stage.icon}
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {stage.modules.map((moduleId) => {
                  ...
                  return (
                    ...
                    >
                      <div className={`bg-gradient-to-r ${module?.color ?? 'from-gray-500 to-gray-600'} p-4 text-white`}>
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 bg-white/20 rounded-lg flex items-center justify-center text-xl">
                            {module?.icon ?? '📚'}
                          </div>
                          <div>
                            <h3 className="font-bold">{module?.title ?? moduleId}</h3>
                            <p className="text-xs text-white/80">{lessons.length}节课</p>
                          </div>
                        </div>
                      </div>
                      <div className="p-4">
            ...
            <div className="space-y-4">
              ...
              <div className="p-4 bg-green-50 rounded-lg border border-green-200">
                ...
                <p className="text-sm text-green-800">
                  按照阶段顺序学习：基础认知 → Prompt工程 → Agent开发 → RAG知识库 → 生产实践
                </p>
              </div>
              <div className="p-4 bg-purple-50 rounded-lg border border-purple-200">
                <h4 className="font-medium text-purple-900 mb-2">💡 学习技巧</h4>
