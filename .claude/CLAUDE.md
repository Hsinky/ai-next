...
- **`(main)`** - 主应用页面。包含固定的 Header 组件和 72px 宽度的固定 Sidebar 组件。内容区域通过 `ml-72` 进行偏移。
...
- **顶部栏** - 固定在顶部，高度 56px
- **侧边栏** - 固定在左侧，宽度 72px，高度 `calc(100vh - 56px)`
- **布局组合**: MainLayout 包裹 Header + Sidebar + 主内容区域

#### 学院/学习模块
位于 `/learn`，包含：
- 三大学习阶段: 入门(beginner)、进阶(developer)、高级(architect)，路径为 `/learn/path/{stage}`
- 能力专属课程: `/learn/{type}/{level}`，type ∈ {skill, prompt, mcp, workflow, agent, knowledge}
- 课程内容定义在 `src/data/courseContent.tsx`，使用 `getCourseContent(type, level)` 函数获取
